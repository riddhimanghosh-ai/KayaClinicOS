...
export function AppointmentsClient({
  ...
  // Doctor unavailability from Clinic Ops (persisted in localStorage)
  const docUnavailable = (() => {
    if (typeof window === "undefined") return [];
    try {
      const all: Array<{ doctorId: number; startDate: string; endDate: string; startTime: string; endTime: string; reason: string }> =
        JSON.parse(localStorage.getItem("kaya_doctor_blocks") ?? "[]");
      return all.filter(b => b.startDate <= date && b.endDate >= date);
    } catch { return []; }
  })();
  ...
  return (
    <div className="space-y-4">
      ...
      {docUnavailable.length > 0 && (
        <div className="rounded-lg border border-amber-200 bg-amber-50 px-4 py-2.5 flex flex-wrap items-center gap-3">
          <AlertTriangle className="h-3.5 w-3.5 text-amber-600 shrink-0" />
          <span className="text-xs font-semibold text-amber-800">Doctor unavailability today:</span>
          {docUnavailable.map((b, i) => {
            const matchedAppt = appts.find(a => a.doctor_name && b.doctorId === (a as any).doctor_id);
            const name = matchedAppt?.doctor_name ?? `Doctor #${b.doctorId}`;
            return (
              <span key={i} className="inline-flex items-center gap-1 rounded-full bg-amber-100 border border-amber-200 px-2.5 py-0.5 text-[11px] font-medium text-amber-800">
                {name.replace(/^Dr\.?\s*/i, "Dr. ")} · {b.startTime}–{b.endTime}
                {b.reason ? ` · ${b.reason}` : ""}
              </span>
            );
          })}
